...
{
	if ( $("#toggleProcess").html() == 'show complete')
	{
		$("#lower").show();
		$("#toggleProcess").html('hide complete');
	}
	else
	{
		$("#lower").hide();
		$("#toggleProcess").html('show complete');
	}
}

function toggleInProcessed()
{
	if ( $("#toggleInProcess").html() == 'show planning')
	{
		$("#upper").show();
		$("#toggleInProcess").html('hide planning');
	}
	else
	{
		$("#upper").hide();
		$("#toggleInProcess").html('show planning');
	}
}
...
<a id='toggleInProcess' href='javascript:toggleInProcessed()'>hide planning</a>&nbsp;&nbsp;
<a id='toggleProcess' href='javascript:toggleProcessed()'>hide complete</a></p>

<div id='project-list'>
<div id='upper'>
<table bordercolor="#888888" width="800" border="1" align="center" cellpadding="6" cellspacing="0">
  <tr style="border:#4d67a0">
    <th width="60" colspan="1" nowrap class="customerBanner" style="font-size:20px; height:20px" scope="col"><span id='upper-expand-link' class='expand-all' onClick="expandAll('upper')">+</span>&nbsp;<span style="float:right"><a href='javascript:hideSection("lower");expandAll("upper", "print")'>print</a></span></th>
