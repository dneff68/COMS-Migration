	...
	elseif (!empty($DELIVERY_DATA) && ($tankAction == 'submit' || $tankAction == 'markcancel'))
	{
		...
		if (!empty($deliveryProduct) && sizeof($_SESSION['DELIVERY_TANKS']) > 0)
		{
			...
			if (!empty($modifyDeliveryID))
			{
				...
				{
					$origLine = mysql_fetch_assoc($origRes);
					extract($origLine);
				}
			...
			*/
			...
			foreach($_SESSION['DELIVERY_TANKS'] as $monitorID)
			{
				...
				if (checkResult($delUnitsRes))
				{
					...
					$delUnitLine = mysql_fetch_assoc($delUnitsRes);
					extract($delUnitLine);
...
//	$DELIVERY_DATA['ratio'] = $ratio;
	...
	{
		$truckCaps = '';
		while ($truckline = mysql_fetch_assoc($truckRes))
		{
			extract($truckline);
...
if (checkResult($res))
{
	...
	while ($line = mysql_fetch_assoc($res))
	{
		...
		{
			$dataLine = mysql_fetch_assoc($dataRes);
			extract($dataLine);
			if ($units == 'Inches')
			{
...
//	}
			...
			if (checkResult($res2))
			{
				while ($line2 = mysql_fetch_assoc($res2))
				{
					extract($line2);
...
if (strpos($fillDate, 'Saturday') !== false || strpos($fillDate, 'Sunday') !== false)
{
	$weekendFormat = " class='spinAlert'";
			...
			if ( $_SESSION['USERTYPE'] != 'customer' )
			{
				...
				{
					$monitorLine = mysql_fetch_assoc($monitorRes);
					extract($monitorLine);
