...
function getTargetDay($dayval, $serializedTargets, $defaultTarget=-1)
{
	...
	$dupeLink = $dayval==7 ? "<a href='javascript:dupeVals()'>update all</a>" : '';

	return "
	<td width='36'>$dow</td>
	  <td width='216'><input name='dosage$dayval' value='$dosage' type='text' id='dosage$dayval' size='5' maxlength='5' onkeypress='return numbersonly(this, event)'/> $dupeLink
	</td>\n";
...
function dupeVals()
{
	firstVal = document.targetForm.dosage7.value;
	if (firstVal == '')
	{
		alert('Please provide a target for Sunday');
		return;
	}

	document.targetForm.dosage1.value = firstVal;
	document.targetForm.dosage2.value = firstVal;
	document.targetForm.dosage3.value = firstVal;
	document.targetForm.dosage4.value = firstVal;
	document.targetForm.dosage5.value = firstVal;
	document.targetForm.dosage6.value = firstVal;
...
<body>
<form action="setTargetDose.php" method="post" id="targetForm" name="targetForm">
