...
$res = getResult("SELECT d.units, d.value as storedValue, t.diameter from monitor m, data d, tank t where d.monitorID=m.monitorID and d.date='$datetime' and m.monitorID='$monitorID' and t.monitorID='$monitorID' LIMIT 1");
if (checkResult($res))
{
...
*/
	...
	else
	{
		...
		executeQuery($query);
		logAction("Reading dated $datetime CHANGED from $storedValue to $txtNewValue for $tankName" );
		generateStats($monitorID, "'$datetime'");
		$js = "\nwindow.close();\n";
	}
...
if (!empty($set_units) && $set_units <> $units)
{
...
<form action="changeReading.php" method="post" name="readingForm" >
...
-->	  </td>
  ...
  <tr align="right"><td colspan="2" style="font-size:smaller"><a href='/changeReading.php?set_units=<?=$units == 'Inches' ? 'Gallons' : 'Inches' ?>&monitorID=<?=$monitorID?>&datetime=<?=$datetime?>'>Toggle to <?=$units == 'Inches' ? 'Gallons' : 'Inches' ?></a></td></tr>
</table>
</form>
<p>&nbsp;</p>
</body>
