		...
		while ($line = $res->fetch_assoc())
		{
			...
			if (checkResult($tres))
			{
				while ($line = mysql_fetch_assoc($tres))
				{
					...
					$stat = checkTankStatus($monitorID, $STATUS_FILTER);
					//$debug .= "$STATUS_FILTER == $stat<BR>";
					$allow = true;
					$mkrcolor = 'green';
					...
					if (!empty($STATUS_FILTER))
					{
						...
						//$debug .= "$monitorID : $statkey == $STATUS_FILTER<br>";
