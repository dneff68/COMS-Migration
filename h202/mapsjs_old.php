...
{
	$query = "SELECT t.concentration as con, p.value
...
function drawSamplePoints($tankID)
{
	...
	google.maps.event.addListener(marker, 'click', function() 
		{ 	
		infoWindow.close();
		// surfDialog(\"/charts/customerGraph.php?monitorID=$tankID\", 835, 550, window, false);	
...
//	error_log($query);
	...
	// get tanks for site
	$query = "SELECT t.tankID, m.LatLng as tankLatLng, tl.position, tl.LatLng 
		FROM tank t, monitor m, trunkLine tl 
		WHERE t.monitorID=tl.tankID AND t.monitorID=m.monitorID AND m.monitorID='$tankID' AND lineNumber=1 ORDER BY tl.position";
	...
	while ( $trunkLineValues = mysql_fetch_assoc($tankInfoResult) )
	{
		...
		{
			echo ("\nvar latlng = new google.maps.LatLng($LatLng);\n"); 
			echo ("\ntrunkLineCoordinates.push(latlng);");
		}
	}
	...
	$trunkColor = "#008800";
	//$query = "SELECT alarm FROM flowAlarm WHERE flowlineID='$flowlineID' ORDER BY date DESC LIMIT 1";
	...
	echo ("\n
	var trunkLinePath = new google.maps.Polyline({
	  path: trunkLineCoordinates,
	...
	echo ("
	google.maps.event.addListener(trunkLinePath, 'click', function(event, index) 
		{ 	infoWindow.position = event.latLng;
			infoWindow.setContent(\"$balloon\");
			infoWindow.close();
			infoWindow.open(map);});		
			trunkLinePath.setMap(map);");
